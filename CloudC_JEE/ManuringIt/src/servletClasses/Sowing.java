...
/**
 * Servlet implementation class Sowing
 */
@WebServlet("/Sowing")
public class Sowing extends HttpServlet {
	...
	protected void service(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		request.getRequestDispatcher("sowing.jsp").include(request, response);
	...
	private ArrayList<String []> list(byte month){
		...
		try {
			...
			String sql="select name,phone,dist,state,crop1,crop2,crop3,c.cid from custinfo c,"+table[c++]+" r where c.cid=r.cid and sowing=?;";
			PreparedStatement pstate =con.prepareStatement(sql);
			pstate.setByte(1, month);
			rs=pstate.executeQuery();
			if(rs==null || !rs.first()){
				sql="select name,phone,dist,state,crop1,crop2,crop3,c.cid from custinfo c,"+table[c++]+" r where c.cid=r.cid and sowing=?;";
