...
public class ViewL3 {
	...
	public int insert(int minSup_count){
		...
		for(int i=0;i<table_size;i++){
			...
			sql="insert into l3 SELECT distinct("+feild+"),count("+feild+") FROM h3 where "+feild+" is not null group by "+feild+" having count(*) >= ?;";
	...
	public static int avg() {
		Connection con=connection_MySQL.ConnectionClass.dbcon();
		String sql="select avg(count) from l3";
		ResultSet rs=null;
		PreparedStatement state;try {
			state = con.prepareStatement(sql);
			rs=state.executeQuery();
			rs.next();
			System.out.println("AVG L3 : "+rs.getFloat("avg(count)"));
